...
{
    outPos = vec3((gl_VertexIndex << 1) & 2, gl_VertexIndex & 2, 0.0);
    gl_Position = vec4(outPos.xy * 2.0f + -1.0f, 0.0f, 1.0f);
}
...
vec2 IntegrateBRDF(float NdotV, float roughness)
{
	...
	return lut / material_ubo.sampleCount;
}

void materialFragment()
{
	outColour = vec4(IntegrateBRDF(inPos.s, 1.0 - inPos.t), 0.0, 1.0);
}
